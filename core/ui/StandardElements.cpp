...
namespace RLGameGUI
{
    ...
    void GUIPanel::Draw(const Texture2D &fill, Color tint, const Rectangle& source, const Vector2& offset, const Vector2& scale)
    {
        ...
        {
            // DrawTexture(fill, source, rect, V2Zero, 0, 1, tint);
        }
    ...
    void GUILabel::OnRender()
    {
        ...
        if (TextFont.texture.id == 0)
            fontToUse = GetFontDefault();
        DrawTextEx(fontToUse, Text.c_str(), {TextRect.x, TextRect.y}, TextSize, Spacing, Tint);
    }
    ...
    void GUIButton::OnRender()
    {
        ...
        if (!Text.empty())
        {
            ...
            if (TextFont.texture.id == 0)
                fontToUse = GetFontDefault();
            DrawTextEx(fontToUse, Text.c_str(), {offset.x, offset.y}, TextSize, Spacing, labelColor);
        }
    }
